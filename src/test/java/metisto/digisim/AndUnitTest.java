...
public class AndUnitTest {
    ...
    public void should_return_true_when_all_input_are_true() {
        assertEquals(true, and(fixed(true), fixed(true)).output().value());
    }

    @Test
    public void should_return_false_when_at_least_one_input_is_false() {
        assertEquals(false, and(fixed(false), fixed(true)).output().value());
        assertEquals(false, and(fixed(true), fixed(false)).output().value());
        assertEquals(false, and(fixed(false), fixed(false)).output().value());
    }
